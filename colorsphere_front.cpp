...
for(int i=0;i<no_sphere;i++)
{
  ...
  glBindVertexArray(0);
	...
	GLfloat timeValue,redValue,greenValue;
	...
	for(int i=0;i<no_sphere;i++)
{
	...
	m[i]+=3;
       ...
       redValue = (cos(timeValue) / 2) + 0.5;
       vertexColorLocation = glGetUniformLocation(shaderprogram,"colour");
	}
...
{
ycord=ycenter+r*sin((phi/180)*3.14);
xcord=xcenter+r*cos((phi/180)*3.14)*cos((theta/180)*3.14);
zcord=zcenter+r*cos((phi/180)*3.14)*sin((theta/180)*3.14);
