...
for(phi=0;phi<=90;phi+=30)
{
for(theta=0;theta<360;theta+=30)
{
zcord=z+r*sin((phi/180)*3.14);
xcord=x+r*cos((phi/180)*3.14)*cos((theta/180)*3.14);
ycord=y+r*cos((phi/180)*3.14)*sin((theta/180)*3.14);
...
cout<<endl<<endl<<vertices[3*16]<<"  "<<vertices[3*16+1]<<"  "<<vertices[3*16+2]<<endl<<endl;
for(i=0;i<=index2-1;i+=3)
{
cout<<indices[i]<<" "<<indices[i+1]<<" "<<indices[i+2]<<endl;
cout<<vertices[3*indices[i]]<<" "<<vertices[3*indices[i]+1]<<" "<<vertices[3*indices[i]+2]<<"   "<<vertices[3*indices[i+1]]<<" "<<vertices[3*indices[i+1]+1]<<" "<<vertices[3*indices[i+1]+2]<<"  "<<vertices[3*indices[i+2]]<<" "<<vertices[3*indices[i+2]+1]<<" "<<vertices[3*indices[i+2]+2]<<endl;
}
  ...
  if (glewInit() != GLEW_OK)
{
 ...
 /* GLfloat vertices[]={
    -0.5f,0.0f,0.0f, //base left
     0.5f,0.0f,0.0f, //base right
     0.0f,0.5f,0.0f, //base top
     0.0f,0.25f,0.5f};//3d top
 ...
 /*The game loop*/
  while(!glfwWindowShouldClose(window))
	...
	//glDrawArrays(GL_TRIANGLES,0,index-1);      
	glBindVertexArray(0);
    }
  glfwTerminate();
  return 0;
